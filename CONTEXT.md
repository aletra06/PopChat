...
Window placement uses an explicit default, initially migrated from the old remembered position. The anchor stores the display ID, horizontal center relative to the display, and distance below its usable top edge. Reopening restores it; dragging alone never overwrites it. The underlined action below the composer saves a new default. During header drags, noninteractive guide windows show the default outline and a vertical display centerline when snapped. Default snapping enters within 18 pt and releases past 30 pt; centerline snapping uses 12/22 pt. Pointer movement is measured from the original unsnapped frame so either target can be left smoothly. Missing displays fall back to an available screen without overwriting the saved anchor.
